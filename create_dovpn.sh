...
if [[ $# -eq 0 ]]
   ...
   if [ "$1" -ge 1194 -a "$1" -le 1204 ]
	then 
		...
		cp -R $HOME/0000/ $HOME/$1

		# create new instance of dovpn
		docker run -ti -d -P --privileged --name dovpn$1 -e PORT="port ${1}" -e REMOTE="remote dovpn.ml ${1}" -p $1:$1/udp --net=bridge -v $HOME/$1:/etc/openvpn:rw rferroni/dovpn /bin/bash
		sleep 3
		...
		# copy client.ovpn
		mkdir /var/nginx/dovpn/config/$1
		cp $HOME/client$1.ovpn /var/nginx/dovpn/config/client$1.ovpn
